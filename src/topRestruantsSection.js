...
let  filteredResult = copyTopRestaurants?.filter((item,i)=>{
  if(item?.info?.name?.toLowerCase().includes( searchTopRestaruants?.toLowerCase())== true){
    return true
  }
})
...
function sortRestaurants(category){
 ...
 filteredResult= copyTopRestaurants?.sort((a,b)=>{
  ...
  setFilteredTopRestaruants(filteredResult)
    ...
    return(<>
     <h2 style={{fontSize:"25px",fontWeight:"700 ",wordSpacing:"0.5px"}}> Restaurants with online food delivery in {yourLocation.slice(0,10)}..</h2>
   <div className="top-restaurants-filter-bar">
    <input className="search-input"   type="text" placeholder="Search top rated restaurants" aria-label="default input example" 
  ...
  <p>Sorry : ) We Cannot find any Top Restaurants with this Name</p> : ""}
    ...
    function Card({item}){
      return(
        ...
        }} className="card top-restaurants-card  "style={{width: "230px"}} >
      ...
      <div className=" card-body card-body2" style={{ }}>
        ...
        <b style={{letterSpacing:"0.5px",fontWeight:"500px"}}>{item?.info?.locality}</b>
      </div>
    </div>
      )
    }
