...
function LocationData( {suggetions,setSuggetions,locations,setTopRestaurants,setGeoLocation,setLocations,yourLocation,setYourLocation}){
...
<input value = {suggetions} onChange={(e)=>{
  setSuggetions(e.target.value)
}} className="location-input" placeholder="Search your city"></input>

   {suggetions != "" && suggetions?.length>=3  ? locations?.map((item,i)=>{
    // console.log(item.description)
    return  <div className="location-result locations"key={i}  data-bs-dismiss="offcanvas" aria-label="Close"
     onClick={()=>{
      setTopRestaurants([])
  ...
  .then((res)=>{
    ...
    }} > <i className="bi bi-geo-alt"></i> {item?.description}.. </div>  
   }):""}</div>
        </>
    )
}
